...
<html lang="en" style="scroll-behavior: smooth;">
      ...
      <div class="container-fluid bg-3 text-center">    
        ...
        <div class="row text-center">
          <div class="col-sm-6 text-right">
            ...
            <label for="RegistraPassword" style="font-size:15px">New Password </label><br>
          </div>
          <div class="col-sm-6 text-left">
            <form id="Cadastro" method="POST">
              ...
              <input type="password" placeholder="Enter New Password" name="password" id="RegistraPassword" required> <input type="checkbox" onchange=MostraSenha()> Show Password <br>
            </form>
          </div>
        </div>
        <div class="row text-center">
            <input  type="button" value="Reset Password" form="Cadastro" onclick=ValidaLogin()> <br> 
          </div>
      </div>
    ...
    <script>
      ...
      { 
        ...
        {
          alert("New Password isn't filled.")
        }
        if(document.getElementById('RegistraNome').value!="" && document.getElementById('RegistraNome').value!="" && document.getElementById('RegistraPassword').value!="")
        {
          ...
          xhttp.onreadystatechange = function() 
          {
              ...
              {
                  //Quando atualizou
                  if(this.responseText=="Same email and name")
                  {
                    //atualiza cadastro
                    var xhttp;
                    xhttp = new XMLHttpRequest();
                    ...
                    xhttp.open("POST", "MudaSenha.php", false); //o false não deixa ser assincrono
                    xhttp.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
                    xhttp.send("email="+document.getElementById('RegistraEmail').value+"&senha="+document.getElementById('RegistraPassword').value);
                  }
                  else if(this.responseText=="Name doesn't match")
                  {
                    alert("Name doesn't match with the registered name")
                  }
                  else
                  {
          ...
          xhttp.open("POST", "LerEmaileNome.php", false); //o false não deixa ser assincrono
          xhttp.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
          xhttp.send("email="+document.getElementById('RegistraEmail').value+"&nome="+document.getElementById('RegistraNome').value);
          }
      }
